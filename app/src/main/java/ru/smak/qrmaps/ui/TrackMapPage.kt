...
private fun MapView.createPath(
    ...
    if (path.isNotEmpty()) {
        ...
        val intImg = ImageProvider.fromBitmap(createBitmap(pointRadius))
        val fstImg = ImageProvider.fromBitmap(createBitmap(pointRadius, Color.GRAY, Color.RED))
        val lastImg = ImageProvider.fromBitmap(createBitmap(pointRadius, Color.BLUE))
...
fun createBitmap(pointRadius: Int, pointFillColor: Int = Color.GREEN, pointStrokeColor: Int = Color.MAGENTA): Bitmap {
    val bmp = Bitmap.createBitmap(pointRadius, pointRadius, Bitmap.Config.ARGB_8888)
    val fp = Paint().apply { color = pointFillColor }
    val sp = Paint().apply { color = pointStrokeColor }
    val c = Canvas(bmp)
